  ...
  var scenarios = [
    {
      ...
      setup: function () {
        return JSON.parse(localStorage.getItem('json') || randomJson)
      },
      test: function (storedObject) {
        storedObject[4].isActive = !storedObject[4].isActive
        localStorage.setItem('json', JSON.stringify(storedObject))
      }
    },
    {
    ...
    {
      name: 'Iterate through localStorage keys',
      setup: function () {
        for (var i = 0; i < 1000; i ++) {
          var key = btoa('' + i + (i + 1) + (i + 2) + (i + 3) + (i + 4) + (i + 5) + (i + 6))
          localStorage.setItem(key, '')
    ...
    {
      name: 'Send event to tab',
      setup: function () {
        localStorage['trigger'] = ['a']
      },
      test: function (_, callback) {
        var tab
  ...
  scenarios.forEach(function (scenario) {
    ...
    button.addEventListener('click', function () {
      ...
      requestAnimationFrame(function () {
        ...
        var setupObj = scenario.setup()
        performance.mark('start')
        if (scenario.async) {
          scenario.test(setupObj, done)
        } else {
          for (var i = 0; i < iterations; i++) {
            scenario.test(setupObj)
          }
          done()
        }
      })
